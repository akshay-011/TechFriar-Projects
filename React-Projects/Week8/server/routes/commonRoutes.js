...
router.post("/login", async (req, res) => {
    ...
    // for admin login
        ...
        if(login){
            ...
            req.session.save(() => {
                return res.json({ data:"Admin login succes "+user.username, username:"Admin", isAdmin:true, user:user }).status(200).send();
            })
        }
        else{
    ...
    // Normal user login
        ...
        if(login){
            ...
            req.session.save(() => {
                return res.status(200).json({ data:"login succes "+user.username, isAdmin:false, user:user }).send();
            });
        }
        else{
